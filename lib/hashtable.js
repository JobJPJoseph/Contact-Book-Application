...
class HashTable {
    ...
    deleteByName(name) {
        let idx = this.hashMod(name[0].toLowerCase());
        this.data[idx].deleteByContext(name);
    }

    deleteByContact(phone) {

        for (let i = 0; i < this.data.length; i++) {
            let tree = this.data[i];

            if (tree) {
                tree.deleteByContext(phone);
            }
        }
    }
    ...
    print() {
        ...
        for (let i = 0; i < this.data.length; i++) {
            ...
            if (tree) {
                ...
                console.log(tree.root.name[0].toUpperCase().underline.white, "\n");
