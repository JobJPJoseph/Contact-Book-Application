...
class ContactSearchTree {
    ...
    searchPhoneNumber(number, branch=this.root) {
        if (this.root === null) return null;

        if (branch === null) return null;

        if (branch.contact === number) return branch;

        let left = this.searchPhoneNumber(number, branch.left);
        let right = this.searchPhoneNumber(number, branch.right);
